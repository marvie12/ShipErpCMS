...
<?php } ?>
	...
	{{ Form::open(array('url' => 'provider/process','method' => 'post','data-abide' => '', 'autocomplete'=>"off")) }}
				...
				<fieldset>
					...
					<div class="<?php if($errors->first('name')!=''){ echo 'error'; } ?>">
						<label>Provider Name
					        ...
					        {{ Form::text('name', Input::old('name'), array('required' => true, 'id' => 'name')) }}
					    </label>
					...
					<div class="<?php if($errors->first('url')!=''){ echo 'error'; } ?>">
						<label>Provider URL
				    		<span class="f2 rqrd"><i>Required!</i></span>
					        {{ Form::text('url', Input::old('url'), array('required' => true, 'id' => 'url')) }}
					    </label>
